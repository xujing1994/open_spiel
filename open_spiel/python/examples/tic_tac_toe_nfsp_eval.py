...
flags.DEFINE_string("save_path", "/home/jxu8/Code_update/open_spiel/evaluation_data/eval_ttt_nfsp_1_7_29/", "Path to load the session")
...
def eval_against_random_agent1(env, trained_agents, random_agents, num_episodes):
  ...
  player_name = ['trained agent0', 'random agent1']
  for _ in range(num_episodes):
      ...
      while not time_step.last():
        ...
        action_string = env.get_state.action_to_string(player_id, agent_output.action)
        print("Player ", player_name[player_id], ", randomly sampled action: ",
              action_string)
        time_step = env.step([agent_output.action])
        print(env.get_state)
      for pid in range(env.game.num_players()):
          print("Utility for player {} is {}".format(player_name[pid], time_step.rewards[pid]))
...
def eval_against_random_agent0(env, trained_agents, random_agents, num_episodes):
  ...
  player_name = ['random agent0', 'trained agent1']
  for _ in range(num_episodes):
      time_step = env.reset()
      print(env.get_state)
      while not time_step.last():
...
def eval_against_trained_agents(env, trained_agents, num_episodes):
  wins = np.zeros(3)
  player_name = ['trained agent0', 'trained agent1']
  for _ in range(num_episodes):
    time_step = env.reset()
    print(env.get_state)
    while not time_step.last():
      ...
      action_string = env.get_state.action_to_string(player_id, agents_output.action)
      print("Player ", player_name[player_id], ", randomly sampled action: ",
            action_string)
...
def main(unused_argv):
  ...
  for counter, dir in enumerate(model_dirs[-10:], len(model_dirs) - 9):
      if counter % 1 == 0:
          ...
          with tf.Session() as sess:
            ...
            win_rates_against_trained_agents = eval_against_trained_agents(env, [agent0, agent1], 10)

            # f1 = open(FLAGS.save_path + "win_rates/" + eta_name + "/win_rates_against_random_agent1.txt", "a")
            ...
            # f2 = open(FLAGS.save_path + "win_rates/" + eta_name + "/win_rates_against_random_agent0.txt", "a")
            ...
            # f3 = open(FLAGS.save_path + "win_rates/" + eta_name + "/win_rates_against_eachother.txt", "a")
